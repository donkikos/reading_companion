...
- [ ] The embedding model matches the model used during ingestion (or is documented if not enforced).
- [ ] Run `ruff format` on changed Python files (line length 100)
- [ ] Run `ruff check .` and ensure it passes
- [ ] Add or update tests for this change
- [ ] Tests pass
...
- [ ] Qdrant queries include a spoiler-safe filter (currently `pos_end <= user_pos`).
...
- [ ] When assembling sentence-based context, iterate `sentences` and compute `sid = pos_start + k`.
...
- [ ] Overlapping sentences across chunks are removed from sentence-based context.
